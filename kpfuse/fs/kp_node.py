...
def create_attribute(meta):
    created_time = get_time(meta.get('create_time'))
    modified_time = get_time(meta.get('modify_time'))
    if meta.get('path') == '/' or meta['type'] == 'folder':
        return DirNodeAttribute(created_time, modified_time)
    else:
        return FileNodeAttribute(meta.get('size', 0), created_time, modified_time)


class DirNodeAttribute(object):
    def __init__(self, created_time=time.time(), modified_time=None):
        self.created_time = created_time
        self.modified_time = modified_time or created_time


class FileNodeAttribute(DirNodeAttribute):
    def __init__(self, size=0, created_time=time.time(), modified_time=None):
        super(FileNodeAttribute, self).__init__(created_time, modified_time)
        self.size = size
...
class AbstractNode(object):
    ...
    def update_meta(self, kp):
        ...
        self.attribute = create_attribute(meta)
...
class DirNode(AbstractNode):
    ...
    def build(self, kp):
        ...
        for x in meta.get('files', []):
            ...
            child_node.attribute = create_attribute(x)
